...
const requestSchema = z.object({
	messages: z
		...
		.min(1),
	model: z.enum(["gpt-4-0613", "gpt-3.5-turbo-0613", "gpt-3.5-turbo-16k-0613"]),
...
export default async function openaiHandler(request: NextRequest) {
	...
	const { messages, model, temperature, presencePenalty, frequencyPenalty, maxTokens } =
		requestParsed.data
						...
						for (const part of parts) {
							if (part !== "[DONE]") {
								try {
									...
									if (typeof partParsed.choices?.[0]?.delta?.content !== "string")
										continue

									const contentDelta = partParsed.choices[0].delta.content
